...
const DetailPenawaran = ({ isOpen, onClose, detailData }) => {
  ...
  const calculateTotals = () => {
    ...
    console.log('📊 SuperAdmin - Grand Total 12 Bulan values:', {
      fromDatabase: {
    ...
    console.log('[SuperAdmin Detail] Discount calculation:');
    ...
    console.log('[SuperAdmin Detail] Grand Total - Disc/Lain2 calculation:');
    ...
    // Use profit from database if available, otherwise calculate
    const profitFromDB = hasilPenawaranData?.profit_dari_hjt_excl_ppn;
    const profitDariHJT = profitFromDB || (grandTotalDiscHargaFinal - grandTotalDiscHargaDasar);
    console.log('[SuperAdmin Detail] Profit dari HJT calculation:');
    console.log('  - Profit from database:', profitFromDB);
    console.log('  - Calculated profit (fallback):', grandTotalDiscHargaFinal - grandTotalDiscHargaDasar);
    console.log('  - Final profit used:', profitDariHJT);
    
    // Use Margin dari HJT from database if available, otherwise calculate
    const marginFromDB = hasilPenawaranData?.margin_dari_hjt;
    const marginDariHJT = marginFromDB || (grandTotalDiscHargaDasar > 0 ? (profitDariHJT / grandTotalDiscHargaDasar) * 100 : 0);
    
    console.log('[SuperAdmin Detail] Margin dari HJT calculation:');
    console.log('  - Margin from database:', marginFromDB);
    console.log('  - Final margin used:', marginDariHJT);
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                ...
                <input
                  type="text"
                  value={detailData?.lokasi_pelanggan || fullDetailData?.lokasi_pelanggan || '-'}
                  readOnly
                  style={{
                    flex: 1,
                    padding: '8px 12px',
