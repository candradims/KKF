  ...
  const fetchPenawaranData = async () => {
    try {
      ...
      if (result.success) {
        ...
        const penawaranWithDetails = await Promise.all(
          result.data.map(async (item) => {
            try {
              console.log(`🔧 Getting detail for penawaran:`, item.id_penawaran);
              
              const detailResult = await penawaranAPI.getById(item.id_penawaran);
              
              if (detailResult.success && detailResult.data) {
                console.log(`🔧 Detail data for ${item.id_penawaran}:`, {
                  hasLayanan: !!detailResult.data.data_penawaran_layanan,
                  jumlahLayanan: detailResult.data.data_penawaran_layanan?.length || 0,
                  tarifAksesList: detailResult.data.data_penawaran_layanan?.map(l => l.tarif_akses) || []
                });
                
                return {
                  ...item,
                  data_penawaran_layanan: detailResult.data.data_penawaran_layanan || []
                };
              } else {
                console.warn(`⚠️ Failed to get detail for ${item.id_penawaran}`);
                return {
                  ...item,
              ...
              console.error(`❌ Error getting detail for ${item.id_penawaran}:`, detailError);
              return {
                ...item,
                data_penawaran_layanan: []
              };
        ...
        const transformedData = penawaranWithDetails.map((item, index) => {
          try {
            console.log(`🔧 [${index}] Transforming item:`, item.id_penawaran);
            console.log(`🔧 [${index}] Data layanan:`, item.data_penawaran_layanan);
            
            if (item.data_penawaran_layanan && item.data_penawaran_layanan.length > 0) {
              console.log(`🔧 [${index}] Jumlah layanan:`, item.data_penawaran_layanan.length);
              item.data_penawaran_layanan.forEach((layanan, idx) => {
                console.log(`🔧 [${index}] Layanan ${idx}:`, {
                  id_layanan: layanan.id_layanan,
                  nama_layanan: layanan.nama_layanan,
                  tarif_akses: layanan.tarif_akses,
                  type: typeof layanan.tarif_akses,
                  isNumber: !isNaN(layanan.tarif_akses),
                  isGreaterThanZero: layanan.tarif_akses > 0,
                  isNotNull: layanan.tarif_akses !== null,
                  isNotZero: layanan.tarif_akses !== 0
                });
              });
            } else {
              console.log(`🔧 [${index}] Tidak ada data layanan atau array kosong`);
            }
            ...
            const hasLayananWithTarifAkses = item.data_penawaran_layanan?.some(layanan => {
              const tarif = layanan.tarif_akses;
              const hasTarif = tarif !== null && tarif !== undefined && tarif > 0;
              console.log(`🔧 [${index}] Layanan ${layanan.nama_layanan} - tarif: ${tarif}, hasTarif: ${hasTarif}`);
              return hasTarif;
            }) || false;

            console.log(`🔧 [${index}] FINAL hasLayananWithTarifAkses:`, hasLayananWithTarifAkses);
            ...
            const transformedItem = {
              ...
              actions: ['view'],
              rawData: item,
              hasLayananWithTarifAkses: hasLayananWithTarifAkses
            };
            ...
            return {
              ...
              actions: ['view'],
              rawData: item,
              hasLayananWithTarifAkses: false
            };
          }
        });

        const totalItems = transformedData.length;
        const withTarifAkses = transformedData.filter(item => item.hasLayananWithTarifAkses).length;
        
        console.log("📊 FINAL Data Summary:");
        console.log("📊 Total items:", totalItems);
        console.log("📊 Dengan tarif akses:", withTarifAkses);
        console.log("📊 Tanpa tarif akses:", totalItems - withTarifAkses);

        const filteredData = transformedData.filter(item => item.hasLayananWithTarifAkses);
        
        setPenawaranData(filteredData);
        console.log("✅ Data penawaran berhasil dimuat:", filteredData.length, 
          "items (hanya yang memiliki tarif akses) dari total", 
          totalItems, "items");
