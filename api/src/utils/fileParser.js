...
export class FileParser {
      ...
      reader.onload = (e) => {
        try {
          ...
          const mappedData = jsonData.map((row) => {
            ...
            mappedRow.nama_user =
              row.nama_user || row.nama || row.name || row.Nama || "";
            mappedRow.email_user =
              row.email_user || row.email || row.Email || "";
            mappedRow.kata_sandi =
              row.kata_sandi || row.password || row.kata_sandi || "default123";
            mappedRow.role_user = row.role_user || row.role || row.Role || "";
            mappedRow.target_nr =
              row.target_nr || row.target || row.target_nr || null;
  ...
  static validateUserData(userData) {
    ...
    if (!userData.role_user || userData.role_user.trim() === "") {
      ...
      !["superAdmin", "admin", "sales", "aktivasi"].includes(userData.role_user)
    ) {
      errors.push("Role harus superAdmin, admin, sales, atau aktivasi");
    }
